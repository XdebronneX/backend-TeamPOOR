...
exports.newOrder = async (req, res, next) => {
    ...
    try {
        const validationErrors = [];
        const user = await UserModel.findById(req.body.user).session(session);
        ...
        const orderItemsIds = await Promise.all(
            req.body.orderItems.map(async (orderItem) => {
                const product = await ProductModel.findById(orderItem.product).populate("brand").session(session);
                ...
                newOrderItem = await newOrderItem.save({ session });
                return newOrderItem._id;
            })
        );
